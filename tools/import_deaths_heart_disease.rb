...
class DeathsHeartDiseaseImporter
  def self.grab_heart_disease_death_data
    inputfile = "#{RAILS_ROOT}/data/tgs00059.tsv"
    ...
    data[1..-1].each do |row|
      ...
      row[1..-1].each_with_index do |cell, i|
        ...
        if rate
          ...
          rate.heart_disease = value
          rate.heart_disease_provisional = provisional
          rate.save
        else
          region.death_rates.create!(:start_year => start_year,
                                     :end_year => end_year,
                                     :heart_disease => value,
                                     :heart_disease_provisional => provisional)
        end
      end #cell loop
    end #row loop
    DeathRate.count
...
> require "#{RAILS_ROOT}/tools/import_deaths_heart_disease"
tbl = DeathsHeartDiseaseImporter.grab_heart_disease_death_data
