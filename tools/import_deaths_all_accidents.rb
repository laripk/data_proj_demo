...
class DeathsAllAccidentsImporter
  def self.grab_all_accidents_death_data
    inputfile = "#{RAILS_ROOT}/data/tgs00060.tsv"
    ...
    data[1..-1].each do |row|
      ...
      row[1..-1].each_with_index do |cell, i|
        ...
        if rate
          ...
          rate.all_accidents = value
          rate.all_accidents_provisional = provisional
          rate.save
        else
          region.death_rates.create!(:start_year => start_year,
                                     :end_year => end_year,
                                     :all_accidents => value,
                                     :all_accidents_provisional => provisional)
        end
      end #cell loop
    end #row loop
    DeathRate.count
...
require "#{RAILS_ROOT}/tools/import_deaths_all_accidents"
tbl = DeathsAllAccidentsImporter.grab_all_accidents_death_data
