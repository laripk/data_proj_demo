...
class DeathsCancerImporter
  def self.grab_cancer_death_data
    inputfile = "#{RAILS_ROOT}/data/tgs00058.tsv"
    ...
    data[1..-1].each do |row|
      ...
      row[1..-1].each_with_index do |cell, i|
        ...
        if rate
          ...
          rate.cancer = value
          rate.cancer_provisional = provisional
          rate.save
        else
          region.death_rates.create!(:start_year => start_year,
                                     :end_year => end_year,
                                     :cancer => value,
                                     :cancer_provisional => provisional)
        end
      end #cell loop
    end #row loop
    DeathRate.count
...
> require "#{RAILS_ROOT}/tools/import_deaths_cancer"
tbl = DeathsCancerImporter.grab_cancer_death_data
